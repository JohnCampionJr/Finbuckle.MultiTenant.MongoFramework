...
public static class SeedService
{
    ...
    private static async Task SetupStore(IMultiTenantStore<MongoTenantInfo> store)
    {
        ...
        await store.AddAsync(new MongoTenantInfo { Id = "tenant-finbuckle-d043favoiaw", Identifier = "finbuckle", Name = "Finbuckle" });
        await store.AddAsync(new MongoTenantInfo { Id = "tenant-initech-341ojadsfa", Identifier = "initech", Name = "Initech LLC", ConnectionString = "mongodb://localhost/samples-tenant-initech" });
        await store.AddAsync(new MongoTenantInfo { Id = "tenant-megacorp-g754dafg", Identifier = "megacorp", Name = "MegaCorp Inc" });
    }

    private static async Task SetupDb(IMultiTenantStore<MongoTenantInfo> store, IConfiguration config)
    {
        var ti = store.GetByIdentifierAsync("finbuckle").Result;
        ...
        ti = store.GetByIdentifierAsync("megacorp").Result;
        ...
        ti = store.GetByIdentifierAsync("initech").Result;
        if (ti.ConnectionString is null) ti.ConnectionString = config.GetConnectionString("DefaultPerTenantConnection");
        conn = new MongoPerTenantConnection(ti);
